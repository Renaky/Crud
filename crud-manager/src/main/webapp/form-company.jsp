			...
			<div class="row">
				...
				<div class="form-group col-md-4">
					<label for="user">Usuário</label> <select id="user"
						...
						<option value="" ${not empty company ? "" : 'selected'}>'Selecione um usuário'</option>

						<c:forEach var="user" items="${users}">
							<option value="${user.getId()}"
								${company.getUser().getId() == user.getId() ? 'selected' : ''}>${user.getName()}</option>
						</c:forEach>
					</select>
				</div>
				<hr />
				<div id="actions" class="row pull-right">
					<div class="col-md-12">
						...
						<button type="submit" class="btn btn-primary">${not empty company ? 'Atualizar' : 'Cadastrar'}
							Empresa</button>
					</div>
				</div>
